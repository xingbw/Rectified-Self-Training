...
def office_load_idx(args):
    ...
    if args.office31 == True:  # and not args.home and not args.visda:
        ...
        s_tr, s_ts = "./data/office-31/{}_list.txt".format(
            s
        ), "./data/office-31/{}_list.txt".format(s)
        ...
        t_tr, t_ts = "./data/office-31/{}_list.txt".format(
            t
        ), "./data/office-31/{}_list.txt".format(t)
        prep_dict = {}
...
def train_target_near1(args):
    ...
    score_bank_aug = torch.randn(num_sample, args.class_num).cuda()
    ...
    with torch.no_grad():
        ...
        for i in range(len(loader)):
            ...
            output_aug = netF.forward(inputs_aug)  # a^t
            output_norm_aug = F.normalize(output_aug)
            outputs_aug = oldC(output_aug)
            outputs_aug = nn.Softmax(-1)(outputs_aug)
            if args.sharp:
                outputs_aug = outputs_aug**2 / ((outputs_aug**2).sum(dim=0))
